...
modeltype = 'lasso'
df_cv = pd.read_csv(path_output + modeltype + '_CV-errors_singlespecies.csv')
...
df_errors.round(5).to_csv(path_output + modeltype + '_test-errors_singlespecies.csv', index=False)

df_params = pd.concat(list_df_params)
df_params = pd.concat((df_pa_test, df_params))
df_params.round(5).to_csv(path_output + modeltype + '_trainvalid-coefficients_singlespecies.csv', index=False)

df_preds = pd.concat(list_df_preds)
df_preds = pd.concat((df_pr_test, df_preds))
df_preds.round(5).to_csv(path_output + modeltype + '_predictions_singlespecies.csv', index=False)
