...
modeltype = 'lasso'
df_cv = pd.read_csv(path_output + modeltype + '_CV-errors_bysmiles.csv')
...
# load test output
#df_e_test = pd.read_csv(path_output + modeltype + '_test-errors_bysmiles.csv')
#df_pa_test = pd.read_csv(path_output + modeltype + '_trainvalid-coefficients_bysmiles.csv')
#df_pr_test = pd.read_csv(path_output + modeltype + '_predictions_bysmiles.csv')
...
for i, param in enumerate(ParameterGrid(param_grid)):
    ...
    X_trainvalid = df_trainvalid.loc[:, model.coef_ != 0].to_numpy()
    X_test = df_test.loc[:, model.coef_ != 0].to_numpy()
    print("after first step of LASSO", X_trainvalid.shape, X_test.shape)

    # exit this loop if no variables were selected
    if X_trainvalid.shape[1] == 0:
    ...
    model_rerun.fit(X_trainvalid, y_trainvalid)

    # predict
    y_tv_pred = model_rerun.predict(X_trainvalid)
    y_test_pred = model_rerun.predict(X_test)
    ...
    df_param = mod.get_model_weights(model_rerun, list_cols=list_features)
    df_param['set'] = 'trainvalid'
    df_param['chem_prop'] = chem_prop
    df_param['tax_pdm'] = tax_pdm 
    df_param['tax_prop'] = tax_prop
    df_param['exp'] = exp 
    df_param = mod._add_params_fold_to_df(df_param, param)
    df_param = mod._add_params_fold_to_df(df_param, hyperparam)
    df_param = mod._add_params_fold_to_df(df_param, hyperparam, 'trainvalid')
...
df_errors.round(5).to_csv(path_output + modeltype + '_test-errors_bysmiles.csv', index=False)

df_params = pd.concat(list_df_params)
df_params = pd.concat((df_pa_test, df_params))
df_params.round(5).to_csv(path_output + modeltype + '_trainvalid-coefficients_bysmiles.csv', index=False)

df_preds = pd.concat(list_df_preds)
df_preds = pd.concat((df_pr_test, df_preds))
df_preds.round(5).to_csv(path_output + modeltype + '_predictions_bysmiles.csv', index=False)
