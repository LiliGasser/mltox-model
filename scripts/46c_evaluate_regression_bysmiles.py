...
def compile_errors(modeltype):
    ...
    df_test = pd.read_csv(path_output + modeltype + '_test-errors_bysmiles.csv')
...
# store errors in a second data frame
df_errors_all = df_errors.copy()
...
row = 1
col = 1
for i, chem_fp in enumerate(list_cols_fps):     # add bars
...
fig.update_layout(title=challenge)
...
if do_store_images:
    fig.write_image(path_figures + '46c_bysmiles_' + challenge + '_' + metric + '-vs-models_validation.pdf')
fig.show()
...
for y in list_tickvals:    # add horizontal lines
    fig.add_hline(y=y, 
                  ...
                  row=row,
                  col=col)
for i, chem_fp in enumerate(list_cols_fps):    # add points
...
if do_store_images:
    fig.write_image(path_figures + '46c_bysmiles_' + challenge + '_rmse-r2-vs-models_test.pdf')
fig.show()
...
for groupsplit in ['totallyrandom', 'occurrence']:
    ...
    if do_store_images:
        g.save(path_figures + '46c_bysmiles_' + groupsplit + '_' + metric + '-vs-models.pdf', facecolor='white')
    print(g)
...
for chem_fp in list_cols_fps:
    ...
    if do_store_images:
        g.save(path_figures + '46c_bysmiles_' + chem_fp + '_' + metric + '-vs-models.pdf', facecolor='white')
    print(g)
...
g = (ggplot(data=df_p, mapping=aes(x='model', y='chem_fp', fill=metric, label=metric))
    ...
    + labs(x='model', y='fingerprint', fill=metric, title="ADORE split by SMILES")
    + theme_tufte()
...
if do_store_images:
    g.save(path_figures + '46c_bysmiles_heatmap_' + metric + '.pdf', facecolor='white')
print(g)
