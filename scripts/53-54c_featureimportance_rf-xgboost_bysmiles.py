...
# load data
df_eco = pd.read_csv(path_data + 'processed/' + challenge + '_mortality_bysmiles.csv', low_memory=False)
...
# sparse GP
#path_output_dir = path_vmoutput_gp + '2024-12-03_bysmiles/'
...
df_p_gp_all = pd.DataFrame()

# LASSO
path_output_dir = path_vmoutput_lasso + '2024-12-03_bysmiles/'
...
# RF
path_output_dir = path_vmoutput_rf + '2024-12-03_bysmiles/'
...
# XGBoost
path_output_dir = path_vmoutput_xgboost + '2024-12-03_bysmiles/'
...
# TODO features importances not run yet
...
df_eco_p = pd.merge(df_eco,
                    df_p[['result_id', 'true', 'lasso_pred', 'rf_pred', 'xgboost_pred']],
...
value_vars = ['lasso_pred', 'rf_pred', 'xgboost_pred']
...
df_plot_long = utils._transform_to_categorical(df_plot_long, 'type', ['LASSO', 'RF', 'XGBoost'])
...
value_vars = ['true', 'lasso_pred', 'rf_pred', 'xgboost_pred']
...
list_cols_models = ['true', 'LASSO', 'RF', 'XGBoost']
...
for chemical in list_chemicals:
    ...
    g = (ggplot(data=df_plot, mapping=aes(x='index_frac', color='model'))
        ...
        + theme(figure_size=(8, 6))
        + labs(title=chemical + ' (ADORE split by SMILES)',
    ...
    g.save(path_figures + '53-54_SSD_' + title_medium + '_' + chem_name + '_bysmiles.pdf')
    print(g) 
