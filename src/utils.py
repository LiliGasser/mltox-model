...
def run_GP(X_train, y_train, 
    ...
    if GP_type == 'sparse':
        ...
        if do_monitor:
            ...
            log_dir_scipy = f"{path_vmoutput}/scipy"
            model_task = ModelToTensorBoard(log_dir_scipy, model)
            lml_task = ScalarToTensorBoard(log_dir_scipy, lambda: model.training_loss(), "training_objective")

            monitor = Monitor(MonitorTaskGroup([lml_task, model_task], period=1))
        ...
        if do_monitor:
            ...
            monitor = Monitor(
                MonitorTaskGroup([lml_task, model_task], period=1), 
            )
...
def get_df_opt(opt_logs, do_print=True):
    ...
    if do_print:
        ...
        pass
#        print('success', opt_logs['success'])
